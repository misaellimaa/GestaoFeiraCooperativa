    ...
    private fun carregarDadosDaFeiraAnterior() {
        viewModelScope.launch {
            try {
                ...
                Log.e("RegistrarSobrasVM", "Erro ao carregar dados iniciais.", e)
                _sobraUiItems.value = emptyList()
            }
        }
    }
    ...
    suspend fun calcularDistribuicao(
        ...
        listaSobrasUi.forEach { itemUi ->
            ...
            if (sobraReal > 0) {
                ...
                val entradasOrigem = entradaRepository.getEntradasForFeira(idFeiraOrigem).first()
                    .filter { it.produtoNumero == produtoNumero }
